...
export default function Items() {
  ...
  useEffect(() => {
    axios
      .get(`http://3.87.223.221:8000/items/${itemID}`)
      .then((res) => {
        if (res.data !== "empty") {
  ...
  const handleSubmitMessage = (event) => {
    ...
    console.log(message);
    axios.post("http://3.87.223.221:8000/message", message).catch((err) => {
      console.log(err);
    });
  };
      ...
      <div className="itemContainer">
        <div className="itemPhotoContainer">
          <img
            ...
            src={`http://3.87.223.221:8000/photos/${itemInfo.photo1Name}`}
            alt=""
          />
        </div>
